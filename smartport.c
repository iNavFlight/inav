...
// tens column slot
			...
			slotcol = 70; 
			// if none of the above modes are active. 0 remains unmodified and means ACCRO/RATE mode is active					
			tmpi += slotcol; 					//commit  collected tens column value
			
// hundreds column slot        
					
			//  Included here is a second subset of all modes ASSUMED TO BE IN THE LIMIT OF THIS SLOT mutually exclusive 
			...
			//  Modes wrongly assumed to be in this slot exclusive with others need simply to be moved to another slot where the assumption holds
			slotcol = 0;
...
// ten thousands column slot
			...
			case FSSP_DATAID_T2         :
                if (sensors(SENSOR_GPS)) {
                    ...
                    if (STATE(GPS_FIX))
                        tmpi += 2000;         \\ restored to help keep the focus on what's more important : the tmp1 changes         		
