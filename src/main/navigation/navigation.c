...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_INITIALIZE(navigationFSMState_t previousState)
{
    ...
    if (platformIsFixedWing() && (posControl.homeDistance < navConfig()->general.min_rth_distance) && !posControl.flags.forcedRTHActivated) {
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_CLIMB_TO_SAFE_ALT(navigationFSMState_t previousState)
{
    ...
    if ((posControl.flags.estPosStatue >= EST_USABLE) || !checkForPositionSensorTimeout() || navConfig()->general.flags.rth_climb_ignore_emerg) {
        const float rthAltitudeMargin = platformIsFixedWing() ?
...
static navigationFSMEvent_t navOnEnteringState_NAV_STATE_RTH_HEAD_HOME(navigationFSMState_t previousState)
{
    ...
    if ((posControl.flags.estPosStatue >= EST_USABLE)) {
        if (isWaypointReached(&posControl.homeWaypointAbove, true)) {
            ...
            if (navConfig()->general.flags.rth_tail_first && !platformIsFixedWing()) {
...
bool isWaypointReached(const navWaypointPosition_t * waypoint, const bool isWaypointHome)
{
    ...
    if (platformIsFixedWing() && isWaypointHome) {
...
void calculateInitialHoldPosition(fpVector3_t * pos)
{
    if (platformIsFixedWing()) { // FIXED_WING
...
{
    return !platformIsFixedWing() && (navGetStateFlags(posControl.navState) & NAV_REQUIRE_THRTILT);
}
...
bool navigationRequiresAngleMode(void)
{
    ...
    return (currentState & NAV_REQUIRE_ANGLE) || ((currentState & NAV_REQUIRE_ANGLE_FW) && platformIsFixedWing());
}
...
bool navigationIsControllingThrottle(void)
{
    ...
    return (stateFlags & (NAV_CTL_ALT | NAV_CTL_EMERG | NAV_CTL_LAUNCH | NAV_CTL_LAND)) || (platformIsFixedWing() && (stateFlags & (NAV_CTL_POS)));
}
