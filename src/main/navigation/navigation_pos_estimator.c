...
static bool estimationCalculateCorrection_Z(estimationContext_t * ctx)
{
    ...
    DEBUG_SET(DEBUG_ALTITUDE, 6, accGetVibrationLevel() * 100);         // Vibration level
    DEBUG_SET(DEBUG_ALTITUDE, 1, posEstimator.est.vel.z);               // Vertical speed estimate
    DEBUG_SET(DEBUG_ALTITUDE, 3, posEstimator.gps.vel.z);               // GPS vertical speed
    DEBUG_SET(DEBUG_ALTITUDE, 5, posEstimator.imu.accelNEU.z);          // Vertical acceleration in earth frame
