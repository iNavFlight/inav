...
STATIC_UNIT_TESTED uint8_t RxTxAddr[RX_TX_ADDR_LEN] = {0xE8, 0xE8, 0xF0, 0xF0, 0xE1};
uint32_t *rxSpiIdPtr;
...
STATIC_UNIT_TESTED bool checkPayload(uint8_t *payload) {
  uint8_t Checksum = 0;
  bool validPacket = false;
  for(int s = 0; s < PAYLOAD_SIZE; s++) {
      Checksum ^= payload[s];
  }
  if(Checksum == payload[8]) validPacket = true;
  return validPacket;
}
