...
#define FPORT_TELEMETRY_MAX_CONSECUTIVE_TELEMETRY_FRAMES 2 // Needed to avoid lost sensors on FPort, see #3198
                ...
                case FPORT_FRAME_TYPE_TELEMETRY_REQUEST:
                    if (frameLength != FPORT_FRAME_PAYLOAD_LENGTH_TELEMETRY_REQUEST) {
                        ...
                        case FPORT_FRAME_ID_NULL:
                            if (!rxDrivenFrameRate) {
                                // RX is not telling us wether it can accept a telemetry frame, so
