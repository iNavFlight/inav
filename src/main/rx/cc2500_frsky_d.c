...
void frSkyDSetRcData(uint16_t *rcData, const uint8_t *packet)
{
    ...
    if (!dataError) {
        ...
        DEBUG_SET(DEBUG_FRSKY, DEBUG_DATA_ERROR_COUNT, ++dataErrorCount);
    }
}

rx_spi_received_e frSkyDHandlePacket(uint8_t * const packet, uint8_t * const protocolState, uint16_t *linkQuality)
{
...
void frSkyDInit(void)
{
    ...
    if (feature(FEATURE_TELEMETRY)) {
        initFrSkyTelemetryExternal(frSkyDTelemetryWriteByte);
