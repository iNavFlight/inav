...
rx_spi_received_e frSkyXHandlePacket(uint8_t * const packet, uint8_t * const protocolState, uint16_t *linkQuality)
{
        ...
        if (rxSpiGetExtiState() && (!frameReceived)) {
            ...
            if (ccLen >= packetLength) {
                ...
                if (isValidPacket(packet)) {
                    ...
                    cc2500setRssiDbm(packet[packetLength - 2], linkQuality);
                ...
                if (!frameReceived) {
                    packetErrors++;
                    DEBUG_SET(DEBUG_FRSKY, DEBUG_DATA_BAD_FRAME, packetErrors);
    ...
    case STATE_RESUME:
        if (cmpTimeUs(micros(), packetTimerUs) > receiveDelayUs + 3700) {
            ...
            missingPackets++;
            DEBUG_SET(DEBUG_FRSKY, DEBUG_DATA_MISSING_PACKETS, missingPackets);
            *protocolState = STATE_DATA;
        }
        break;
    }
