...
#define CRSF_TELEMETRY_STALE_LINK_US 1000000
                    ...
                    switch (link->frame.frame.type)
                    {
                        ...
                        case CRSF_FRAMETYPE_MSP_WRITE: {
                            if (link->frame.frame.frameLength >= 4) {
                                const uint8_t *frameStart = (const uint8_t *)&link->frame.frame.payload + CRSF_FRAME_ORIGIN_DEST_SIZE;
                                bufferCrsfMspFrame(link->link, link->frame.frame.payload[1], frameStart, link->frame.frame.frameLength - 4);
                            } else {
    ...
    if (link->frameDone) {
        ...
        if (link->frame.frame.type == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) {
            ...
            if (statistics) {
                ...
                statistics->downlinkLQ = linkStats->downlinkLQ;
                ...
                rxLinkStatisticsUpdated(link->link,
                    RX_LINK_STATS_UPLINK_RSSI | RX_LINK_STATS_UPLINK_LQ | RX_LINK_STATS_DOWNLINK_LQ |
                    RX_LINK_STATS_UPLINK_SNR | RX_LINK_STATS_RF_MODE | RX_LINK_STATS_UPLINK_TX_POWER |
                    RX_LINK_STATS_ACTIVE_ANTENNA);
            }
...
{
    if ((unsigned)linkIndex >= RX_LINK_COUNT) {
        return;
    }

    crsfLinkState_t *link = &crsfLinkStates[linkIndex];
    if (!link->port || link->telemetryBufLen != 0) {
        return;
    }
...
static void crsfRxSendTelemetryDataForLink(crsfLinkState_t *link)
{
    if (link->telemetryBufLen > 0) {
        ...
        if (link->halfDuplex) {
            ...
            if (timeSinceStartOfFrame > CRSF_TELEMETRY_STALE_LINK_US) {
                // A vanished receiver must not hold a telemetry buffer forever.
                // Dropping stale telemetry also allows another CRSF link to keep
                // servicing MSP and normal telemetry independently.
...
{
    return (unsigned)link < RX_LINK_COUNT && crsfLinkStates[link].telemetryBufLen == 0;
}
...
{
#if defined(USE_MSP_OVER_TELEMETRY)
    // Initialize MSP endpoint state before the UART is opened, so an immediate
    // receiver request cannot arrive before its parser context exists.
    initCrsfMspBuffer();
#endif
    ...
    link->telemetryBufLen = 0;
    link->port = NULL;
    link->halfDuplex = tristateWithDefaultOffIsActive(rxConfig->halfDuplex);
    link->link = linkIndex;
    ...
    link->port = openSerialPort(portConfig->identifier,
        ...
        CRSF_BAUDRATE,
        CRSF_PORT_MODE,
        CRSF_PORT_OPTIONS | (link->halfDuplex ? SERIAL_BIDIR : 0)
        );
...
bool crsfBindLink(rxLink_e rxLink)
{
    if ((unsigned)rxLink >= RX_LINK_COUNT || !crsfLinkStates[rxLink].port) {
        return false;
    }

    static const uint8_t bindFrame[] = {
        CRSF_SYNC_BYTE,
        0x07,
    ...
    serialWriteBuf(crsfLinkStates[rxLink].port, bindFrame, sizeof(bindFrame));
    return true;
}

void crsfBind(void)
{
    crsfBindLink(rxGetActiveLink());
}
