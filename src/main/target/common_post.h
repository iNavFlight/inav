...
// MCU_FLASH_SIZE is a poor proxy for RAM (e.g. STM32F405 has 1 MB flash but only ~192 KB
// RAM), so gate on the MCU family/subfamily instead. Enabled targets (>= ~320 KB RAM):
// STM32H7 (512 KB+), STM32F745/F746 (320 KB), STM32F765 (512 KB), AT32F43x (384 KB).
#if defined(USE_BARO) && defined(USE_SDCARD) && !defined(USE_TERRAIN) && ( \
        defined(STM32H7) || \
        defined(AT32F43x) || \
        defined(STM32F745xx) || \
        defined(STM32F746xx) || \
