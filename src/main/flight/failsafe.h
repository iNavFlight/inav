...
typedef struct failsafeConfig_s {
    ...
    uint16_t failsafe_min_altitude;              // Minimum altitude required for failsafe procedure to be taken. 1 step = 1 centimeter. 0 = Regular failsafe_procedure always active (default)
    uint8_t failsafe_min_altitude_procedure;     // selected minimum altitude failsafe procedure is 0: auto-landing, 1: Drop it, 2: Return To Home (RTH)
} failsafeConfig_t;
