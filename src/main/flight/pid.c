...
{
    if (platformUseFwPidController() && currentControlRateProfile->throttle.fixedWingTauMs > 0) {
...
void updatePIDCoefficients(void)
{
    ...
    const float tpaFactor = platformUseFwPidController() ? calculateFixedWingTPAFactor(prevThrottle) : calculateMultirotorTPAFactor();
