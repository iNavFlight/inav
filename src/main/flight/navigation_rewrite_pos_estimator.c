...
void onNewGPSData(void)
{
    ...
    if (sensors(SENSOR_GPS)) {
        if (!(STATE(GPS_FIX) && gpsSol.numSat >= navConfig.inav.gps_min_sats)) {
        ...
        if (navConfig.inav.automatic_mag_declination && !magDeclinationSet) {
        ...
        // FIXME: use HDOP here
        if ((posControl.gpsOrigin.valid) || (gpsSol.numSat >= navConfig.inav.gps_min_sats)) {
            ...
            if (!isFirstGPSUpdate) {
                ...
                if (navConfig.inav.use_gps_velned && gpsSol.flags.validVelNE) {
                ...
                if (navConfig.inav.use_gps_velned && gpsSol.flags.validVelD) {
...
static void updateBaroTopic(uint32_t currentTime)
{
    ...
    if (updateTimer(&baroUpdateTimer, HZ2US(INAV_BARO_UPDATE_RATE), currentTime)) {
        ...
        if (sensors(SENSOR_BARO) && isBaroCalibrationComplete()) {
            ...
            posEstimator.baro.epv = navConfig.inav.baro_epv;
...
static void updateIMUTopic(void)
{
    ...
    if (!isImuReady()) {
        ...
        if (!ARMING_FLAG(ARMED) && navConfig.inav.accz_unarmed_cal) {
...
static void updateEstimatedTopic(uint32_t currentTime)
{
    ...
    if (!isImuReady()) {
        posEstimator.est.eph = navConfig.inav.max_eph_epv + 0.001f;
        posEstimator.est.epv = navConfig.inav.max_eph_epv + 0.001f;
        return;
    }

    /* increase EPH/EPV on each iteration */
    if (posEstimator.est.eph <= navConfig.inav.max_eph_epv) {
        posEstimator.est.eph *= 1.0f + dt;
    }

    if (posEstimator.est.epv <= navConfig.inav.max_eph_epv) {
    ...
    int gpsHistoryIndex = (posEstimator.history.index - 1) - constrain(((int)navConfig.inav.gps_delay_ms / (1000 / INAV_POSITION_PUBLISH_RATE_HZ)), 0, INAV_HISTORY_BUF_SIZE - 1);
    if (gpsHistoryIndex < 0) {
        gpsHistoryIndex += INAV_HISTORY_BUF_SIZE;
    }

    /* Correct accelerometer bias */
    if (navConfig.inav.w_acc_bias > 0) {
        accelBiasCorr.V.X = 0;
        accelBiasCorr.V.Y = 0;
        accelBiasCorr.V.Z = 0;
        ...
        if (isGPSValid) {
            accelBiasCorr.V.X -= (posEstimator.gps.pos.V.X - posEstimator.history.pos[gpsHistoryIndex].V.X) * sq(navConfig.inav.w_xy_gps_p);
            accelBiasCorr.V.X -= (posEstimator.gps.vel.V.X - posEstimator.history.vel[gpsHistoryIndex].V.X) * navConfig.inav.w_xy_gps_v;
            accelBiasCorr.V.Y -= (posEstimator.gps.pos.V.Y - posEstimator.history.pos[gpsHistoryIndex].V.Y) * sq(navConfig.inav.w_xy_gps_p);
            accelBiasCorr.V.Y -= (posEstimator.gps.vel.V.Y - posEstimator.history.vel[gpsHistoryIndex].V.Y) * navConfig.inav.w_xy_gps_v;

            if (useGpsZ) {
                accelBiasCorr.V.Z -= (posEstimator.gps.pos.V.Z - posEstimator.history.pos[gpsHistoryIndex].V.Z) * sq(navConfig.inav.w_z_gps_p);
                accelBiasCorr.V.Z -= (posEstimator.gps.vel.V.Z - posEstimator.history.vel[gpsHistoryIndex].V.Z) * navConfig.inav.w_z_gps_v;
            }
        }

        /* accelerometer bias correction for baro */
        if (isBaroValid && !isAirCushionEffectDetected) {
            accelBiasCorr.V.Z -= (posEstimator.baro.alt - posEstimator.est.pos.V.Z) * sq(navConfig.inav.w_z_baro_p);
        }
        ...
        /* Correct accel bias */
        posEstimator.imu.accelBias.V.X += accelBiasCorr.V.X * navConfig.inav.w_acc_bias * dt;
        posEstimator.imu.accelBias.V.Y += accelBiasCorr.V.Y * navConfig.inav.w_acc_bias * dt;
        posEstimator.imu.accelBias.V.Z += accelBiasCorr.V.Z * navConfig.inav.w_acc_bias * dt;
    }

    /* Estimate Z-axis */
    if ((posEstimator.est.epv < navConfig.inav.max_eph_epv) || useGpsZ || isBaroValid) {
        ...
        if (isBaroValid) {
            ...
            inavFilterCorrectPos(Z, dt, baroError, navConfig.inav.w_z_baro_p);
        ...
        if (useGpsZ) {
            inavFilterCorrectPos(Z, dt, posEstimator.gps.pos.V.Z - posEstimator.history.pos[gpsHistoryIndex].V.Z, navConfig.inav.w_z_gps_p);
            inavFilterCorrectVel(Z, dt, posEstimator.gps.vel.V.Z - posEstimator.history.vel[gpsHistoryIndex].V.Z, navConfig.inav.w_z_gps_v);
        ...
        inavFilterCorrectVel(Z, dt, 0.0f - posEstimator.est.vel.V.Z, navConfig.inav.w_z_res_v);
    }

    /* Estimate XY-axis only if heading is valid (X-Y acceleration is North-East)*/
    if ((posEstimator.est.eph < navConfig.inav.max_eph_epv) || isGPSValid) {
        ...
        if (isGPSValid) {
            inavFilterCorrectPos(X, dt, posEstimator.gps.pos.V.X - posEstimator.history.pos[gpsHistoryIndex].V.X, navConfig.inav.w_xy_gps_p);
            inavFilterCorrectPos(Y, dt, posEstimator.gps.pos.V.Y - posEstimator.history.pos[gpsHistoryIndex].V.Y, navConfig.inav.w_xy_gps_p);

            inavFilterCorrectVel(X, dt, posEstimator.gps.vel.V.X - posEstimator.history.vel[gpsHistoryIndex].V.X, navConfig.inav.w_xy_gps_v);
            inavFilterCorrectVel(Y, dt, posEstimator.gps.vel.V.Y - posEstimator.history.vel[gpsHistoryIndex].V.Y, navConfig.inav.w_xy_gps_v);
        ...
        inavFilterCorrectVel(X, dt, 0.0f - posEstimator.est.vel.V.X, navConfig.inav.w_xy_res_v);
        inavFilterCorrectVel(Y, dt, 0.0f - posEstimator.est.vel.V.Y, navConfig.inav.w_xy_res_v);
    }
