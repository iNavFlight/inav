...
#define SERVO_AUTOTRIM_ATITTUDE_LIMIT   50       // 5 degrees
...
void processContinuousServoAutotrim(const float dT)
{
    ...
    if (ARMING_FLAG(ARMED)) {
        ...
        if ((millis() - lastUpdateTimeMs) > 500) {
            ...
            const bool planeIsFlyingLevel = ABS(attitude.values.pitch + DEGREES_TO_DECIDEGREES(getFixedWingLevelTrim())) <= SERVO_AUTOTRIM_ATITTUDE_LIMIT 
                                            && ABS(attitude.values.roll) <= SERVO_AUTOTRIM_ATITTUDE_LIMIT;
            if (
