...
//                               01234567890123
    ...
    buf[0] = '*';
    buf[1] = ' ';
    buf[2] = osdInfo.bandLetter;
    buf[3] = osdInfo.channelName[0];
    buf[4] = ' ';

    if (osdInfo.frequency)
        tfp_sprintf(&buf[5], "%4d", osdInfo.frequency);
    else
        tfp_sprintf(&buf[5], "----");
    ...
    if (osdInfo.powerIndex) {
        ...
        if (osdInfo.powerMilliwatt) {
            tfp_sprintf(&buf[9], " %4d", osdInfo.powerMilliwatt);
        }
        else {
            tfp_sprintf(&buf[9], " PL=%c", osdInfo.powerIndex);
        }
    } else {
        tfp_sprintf(&buf[9], " ----");
    }
