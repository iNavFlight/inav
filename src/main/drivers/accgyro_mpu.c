...
static void mpu6050FindRevision(gyroDev_t *gyro)
{
    ...
    ack = gyro->mpuConfiguration.readFn(gyro->mpuSpiCsPin, MPU_RA_XA_OFFS_H, 6, readBuffer);
    ...
    if (revision) {
        ...
        ack = gyro->mpuConfiguration.readFn(gyro->mpuSpiCsPin, MPU_RA_PRODUCT_ID, 1, &productId);
...
static bool mpuReadRegisterI2C(IO_t csPin, uint8_t reg, uint8_t length, uint8_t* data)
{
    UNUSED(csPin);
...
static bool mpuWriteRegisterI2C(IO_t csPin, uint8_t reg, uint8_t data)
{
    UNUSED(csPin);
...
bool mpuAccRead(accDev_t *acc)
{
    ...
    bool ack = acc->mpuConfiguration.readFn(acc->mpuSpiCsPin, MPU_RA_ACCEL_XOUT_H, 6, data);
    if (!ack) {
        return false;
    }
...
bool mpuGyroRead(gyroDev_t *gyro)
{
    ...
    const bool ack = gyro->mpuConfiguration.readFn(gyro->mpuSpiCsPin, gyro->mpuConfiguration.gyroReadXRegister, 6, data);
    if (!ack) {
        return false;
    }
...
#ifdef USE_SPI
static bool detectSPISensorsAndUpdateDetectionResult(gyroDev_t *gyro, IO_t mpuSpiCsPin)
{
#ifdef USE_GYRO_SPI_MPU6000
#ifdef MPU6000_CS_PIN
    gyro->mpuSpiCsPin = mpuSpiCsPin == IO_NONE ? IOGetByTag(IO_TAG(MPU6000_CS_PIN)) : mpuSpiCsPin;
#else
    UNUSED(mpuSpiCsPin);
#endif
    if (mpu6000SpiDetect(gyro->mpuSpiCsPin)) {
    ...
    gyro->mpuSpiCsPin = mpuSpiCsPin == IO_NONE ? IOGetByTag(IO_TAG(MPU6500_CS_PIN)) : mpuSpiCsPin;
    if (mpu6500SpiDetect(gyro->mpuSpiCsPin)) {
    ...
    gyro->mpuSpiCsPin = mpuSpiCsPin == IO_NONE ? IOGetByTag(IO_TAG(MPU9250_CS_PIN)) : mpuSpiCsPin;
    if (mpu9250SpiDetect(gyro->mpuSpiCsPin)) {
    ...
    UNUSED(mpuSpiCsPin);
    if (icm20608SpiDetect(gyro->mpuCsPin)) {
    ...
    UNUSED(mpuSpiCsPin);
    if (icm20689SpiDetect(gyro->mpuCsPin)) {
...
void mpuDetect(gyroDev_t *gyro, IO_t mpuSpiCsPin)
{
...
#ifdef USE_SPI
        bool detectedSpiSensor = detectSPISensorsAndUpdateDetectionResult(gyro, mpuSpiCsPin);
