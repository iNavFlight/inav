...
static bool canardSTM32ComputeTimings(const uint32_t target_bitrate, struct Timings *out_timings)
{
    ...
    if (!canardComputeCanTimingSolution(HAL_RCC_GetPCLK1Freq(), target_bitrate, &sol)) {
        return false;
    }
    ...
    out_timings->bs1 = sol.bs1 - 1;  // The HAL does not take care of the 1 bs offset in the register so remove it here like AP does.
    out_timings->bs2 = sol.bs2 - 1;  // The HAL does not take care of the 1 bs offset in the register so remove it here like AP does.
