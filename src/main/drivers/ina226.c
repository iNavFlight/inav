...
int16_t ina226ShuntVoltageToCentiamps(int16_t rawShuntVoltage, uint32_t shuntMicroOhm)
{
    ...
    const int64_t centiAmps = (int64_t)rawShuntVoltage * 250 / shuntMicroOhm;
...
static bool ina226I2cBusIsEnabled(I2CDevice i2cBus)
{
    switch (i2cBus) {
...
{
    if (!dev || i2cBus < 1 || i2cBus > I2CDEV_COUNT || i2cAddress < 0x40 || i2cAddress > 0x4F) {
        return false;
    }

    const I2CDevice i2cDevice = (I2CDevice)(i2cBus - 1);
    if (!ina226I2cBusIsEnabled(i2cDevice)) {
        return false;
    }
    ...
    dev->busDev->busdev.i2c.i2cBus = i2cDevice;
    dev->busDev->busdev.i2c.address = i2cAddress;
