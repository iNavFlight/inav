...
static void processMspPacket(void)
{
    ...
    if (mspFcProcessCommand(&requestPacket, &responsePacket, &mspPostProcessFn) == MSP_RESULT_ERROR) {
...
bool sendMspReply(const uint8_t payloadSizeMax, mspResponseFnPtr responseFn)
{
    ...
    const int chunkRemainder = sbufBytesRemaining(payloadBuf); // free space remainder for current chun
