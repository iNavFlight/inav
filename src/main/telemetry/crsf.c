...
static bool handleCrsfMspFrameBuffer(void)
{
...
static void crsfSendMspResponse(uint8_t *payload, const uint8_t payloadSize)
{
    ...
    crsfSerialize8(dst, CRSF_FRAMETYPE_MSP_RESP); // CRSF type
    crsfSerialize8(dst, mspRequestOriginID);   // response destination must be the same as request origin in order to response reach proper destination.
    crsfSerialize8(dst, CRSF_ADDRESS_FLIGHT_CONTROLLER); // origin is always this device
