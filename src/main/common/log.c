...
void _logBufferHex(logTopic_e topic, unsigned level, const void *buffer, size_t size)
{
    ...
    // 0xAB[space|\n]
    enum {
        LOG_HEX_CHARS_PER_BYTE = 5,
        LOG_HEX_MAX_BYTES = 8,
        LOG_HEX_BUFFER_SIZE = LOG_PREFIX_FORMATTED_SIZE + LOG_HEX_CHARS_PER_BYTE * LOG_HEX_MAX_BYTES + 1,
    };
    char buf[LOG_HEX_BUFFER_SIZE]; // +1 for the null terminator
