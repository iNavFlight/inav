...
TEST(BatteryINA226, CompilesInWithoutChangingAdcDefaults)
{
    EXPECT_EQ(VOLTAGE_SENSOR_ADC, pgResetTemplate_batteryMetersConfig.voltage.type);
    EXPECT_EQ(CURRENT_SENSOR_ADC, pgResetTemplate_batteryMetersConfig.current.type);
    EXPECT_EQ(1, pgResetTemplate_batteryMetersConfig.ina226.i2cBus);
    EXPECT_EQ(INA226_DEFAULT_I2C_ADDRESS, pgResetTemplate_batteryMetersConfig.ina226.i2cAddress);
}
...
static void resetBatteryTestState(void)
{
    ...
    *batteryMetersConfigMutable() = pgResetTemplate_batteryMetersConfig;
    ...
    batteryMetersConfigMutable()->ina226.i2cBus = 1;
    batteryMetersConfigMutable()->ina226.i2cAddress = INA226_DEFAULT_I2C_ADDRESS;
...
TEST(BatteryINA226, AppliesConfiguredI2cBusAndAddress)
{
    fakeFeatures = FEATURE_VBAT | FEATURE_CURRENT_METER;
    fakeTimeUs = 0;
    memset(&systemConfig_System, 0, sizeof(systemConfig_System));
    ...
    *batteryMetersConfigMutable() = pgResetTemplate_batteryMetersConfig;
    ...
    batteryMetersConfigMutable()->current.type = CURRENT_SENSOR_INA226;
    batteryMetersConfigMutable()->ina226.i2cBus = 1;
    batteryMetersConfigMutable()->ina226.i2cAddress = 0x45;

    resetFakeBus();
    batteryInit();

    EXPECT_EQ(I2CDEV_1, fakeBusDevice.busdev.i2c.i2cBus);
    EXPECT_EQ(0x45, fakeBusDevice.busdev.i2c.address);
}
...
TEST(BatteryINA226, IgnoresAdcVoltageOffsetForIna226Current)
{
    resetBatteryTestState();
