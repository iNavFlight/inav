...
|  failsafe_min_altitude  | 0 | If failsafe happens when craft is lower than this altitude in centimeters, failsafe will not execute regular failsafe_procedure, but will execute procedure specified in failsafe_min_altitude_procedure instead. Note: failsafe_min_distance has precedence. 0 = Normal failsafe_procedure always taken. |
|  failsafe_min_altitude_procedure  | DROP | What failsafe procedure to initiate in Stage 2 when craft has lower altitude than failsafe_min_altitude. See [Failsafe documentation](Failsafe.md#failsafe_throttle). |
