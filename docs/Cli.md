...
|  nav_include_waypoint_alt | OFF | Setting to ON will include the altitude in the check if a waypoint is reached. Setting it off will only include the position, but not the altitude. Although even when turned off the aircraft will try to get to the correct altitude. This feature is not supported on fixed wing platforms.|
