...
| nav_fw_launch_allow_throttle_low | OFF |  |  | Allow launch sequence with throttle maintained low throughout. When main launch sequence completes flight control is maintained with Nav cruise throttle until sticks moved/throttle raised or control switches to other Nav mode if preselected |
