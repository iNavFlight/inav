...
# 56 "./src/main/config/config_streamer.c"
void config_streamer_init(config_streamer_t *c)
{
    memset(c, 0, sizeof(*c));
}
...
void config_streamer_start(config_streamer_t *c, uintptr_t base, int size)
{
    ...
    if (!c->unlocked) {
        ...
        c->unlocked = 
# 72 "./src/main/config/config_streamer.c" 3 4
                     1
# 72 "./src/main/config/config_streamer.c"
                         ;
    }
...
# 179 "./src/main/config/config_streamer.c"
...
# 237 "./src/main/config/config_streamer.c"
    if (c->address % ((uint32_t)0x4000) == 0) {

        const FLASH_Status status = FLASH_EraseSector(getFLASHSectorForEEPROM(), ((uint8_t)0x02));
...
int config_streamer_finish(config_streamer_t *c)
{
    if (c->unlocked) {
        ...
        c->unlocked = 
# 292 "./src/main/config/config_streamer.c" 3 4
                     0
# 292 "./src/main/config/config_streamer.c"
                          ;
    }
    return c->err;
}
