...
# 22 "./src/main/rx/sbus.h" 3 4
_Bool 
# 22 "./src/main/rx/sbus.h"
...
# 24 "./src/main/rx/spektrum.h" 3 4
_Bool 
# 24 "./src/main/rx/spektrum.h"
...
# 83 "./src/main/rx/crsf.h" 3 4
_Bool 
# 83 "./src/main/rx/crsf.h"
    crsfRxInit(const struct rxConfig_s *initialRxConfig, struct rxRuntimeConfig_s *rxRuntimeConfig);

# 84 "./src/main/rx/crsf.h" 3 4
_Bool 
# 84 "./src/main/rx/crsf.h"
    crsfRxIsActive(void);
...
int16_t rcRaw[18];
int16_t rcData[18];
timeMs_t rcInvalidPulsPeriod[18];
...
extern const rxConfig_t pgResetTemplate_rxConfig; rxConfig_t rxConfig_System; rxConfig_t rxConfig_Copy; extern const pgRegistry_t rxConfig_Registry; const pgRegistry_t rxConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 24 | (6 << 12), .size = sizeof(rxConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&rxConfig_System, .copy = (uint8_t*)&rxConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_rxConfig}, };


#define RX_SPI_DEFAULT_PROTOCOL 0
# 116 "./src/main/rx/rx.c"
#define RX_MIN_USEX 885
const rxConfig_t pgResetTemplate_rxConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .receiverType = RX_TYPE_SERIAL, .rcmap = {0, 1, 3, 2}, .halfDuplex = 0, .serialrx_provider = SERIALRX_SBUS, .rx_spi_protocol = 0, .spektrum_sat_bind = 0, .serialrx_inverted = 0, .mincheck = 1100, .maxcheck = 1900, .rx_min_usec = 885, .rx_max_usec = 2115, .rssi_channel = 0, .rssiMin = 0, .rssiMax = 100, .sbusSyncInterval = 3000, .rcSmoothing = 1, }
# 134 "./src/main/rx/rx.c"
 ;
...
void resetAllRxChannelRangeConfigurations(void)
{
    ...
    for (int i = 0; i < 4; i++) {
        rxChannelRangeConfigsMutable(i)->min = 1000;
        rxChannelRangeConfigsMutable(i)->max = 2000;
    }
}
...
static 
# 171 "./src/main/rx/rx.c" 3 4
      _Bool 
# 171 "./src/main/rx/rx.c"
...
# 178 "./src/main/rx/rx.c" 3 4
_Bool 
# 178 "./src/main/rx/rx.c"
    serialRxInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig)
{
    
# 180 "./src/main/rx/rx.c" 3 4
   _Bool 
# 180 "./src/main/rx/rx.c"
        enabled = 
# 180 "./src/main/rx/rx.c" 3 4
                  0
# 180 "./src/main/rx/rx.c"
...
void rxInit(void)
{
    ...
    rxRuntimeConfig.rxSignalTimeout = (1000000 / 10);
    rxRuntimeConfig.requireFiltering = 
# 242 "./src/main/rx/rx.c" 3 4
                                      0
# 242 "./src/main/rx/rx.c"
    ...
    for (int i = 0; i < 18; i++) {
        rcData[i] = (1000 + ((2000 - 1000) / 2));
        rcInvalidPulsPeriod[i] = millis() + 300;
    }

    rcData[THROTTLE] = (feature(FEATURE_3D)) ? (1000 + ((2000 - 1000) / 2)) : rxConfig()->rx_min_usec;


    for (int i = 0; i < 20; i++) {
        if (modeActivationConditions(i)->modeId == BOXARM && ((&modeActivationConditions(i)->range)->startStep < (&modeActivationConditions(i)->range)->endStep)) {

            uint16_t value;
            if (modeActivationConditions(i)->range.startStep > 0) {
                value = (900 + 25 * (modeActivationConditions(i)->range.startStep - 1));
            } else {
                value = (900 + 25 * (modeActivationConditions(i)->range.endStep + 1));
            }

            rcData[modeActivationConditions(i)->auxChannelIndex + 4] = value;
        }
    }
...
# 337 "./src/main/rx/rx.c" 3 4
   _Bool 
# 337 "./src/main/rx/rx.c"
        serialProtocolSupportsRSSI = 
# 337 "./src/main/rx/rx.c" 3 4
                                     0
# 337 "./src/main/rx/rx.c"
                                          ;
...
# 381 "./src/main/rx/rx.c" 3 4
_Bool 
# 381 "./src/main/rx/rx.c"
    rxIsReceivingSignal(void)
{
...
# 386 "./src/main/rx/rx.c" 3 4
_Bool 
# 386 "./src/main/rx/rx.c"
    rxAreFlightChannelsValid(void)
{
...
# 405 "./src/main/rx/rx.c" 3 4
_Bool 
# 405 "./src/main/rx/rx.c"
    rxUpdateCheck(timeUs_t currentTimeUs, timeDelta_t currentDeltaTime)
{
    ...
    if (rxSignalReceived) {
        if (currentTimeUs >= needRxSignalBefore) {
            rxSignalReceived = 
# 411 "./src/main/rx/rx.c" 3 4
                              0
# 411 "./src/main/rx/rx.c"
                                   ;
        }
    }
    ...
    if (frameStatus & RX_FRAME_COMPLETE) {
        rxDataProcessingRequired = 
# 417 "./src/main/rx/rx.c" 3 4
                                  1
# 417 "./src/main/rx/rx.c"
                                      ;
...
# 424 "./src/main/rx/rx.c"
    ...
    if (cmpTimeUs(currentTimeUs, rxNextUpdateAtUs) > 0) {
        rxDataProcessingRequired = 
# 428 "./src/main/rx/rx.c" 3 4
                                  1
# 428 "./src/main/rx/rx.c"
                                      ;
    }
...
{
    static int16_t rcSamples[18][5];
    static 
# 438 "./src/main/rx/rx.c" 3 4
          _Bool 
# 438 "./src/main/rx/rx.c"
               rxSamplesCollected = 
# 438 "./src/main/rx/rx.c" 3 4
                                    0
# 438 "./src/main/rx/rx.c"
    ...
    rcSamples[chan][rcSampleIndex % 5] = sample;
...
# 479 "./src/main/rx/rx.c"
    ...
    rxDataProcessingRequired = 
# 482 "./src/main/rx/rx.c" 3 4
                              0
# 482 "./src/main/rx/rx.c"
                                   ;
    ...
    if (skipRxSamples) {
        ...
        return 
# 491 "./src/main/rx/rx.c" 3 4
              1
# 491 "./src/main/rx/rx.c"
                  ;
    }

    rxFlightChannelsValid = 
# 494 "./src/main/rx/rx.c" 3 4
                           1
# 494 "./src/main/rx/rx.c"
                               ;


    for (int channel = 0; channel < rxRuntimeConfig.channelCount; channel++) {
        const uint8_t rawChannel = calculateChannelRemapping(rxConfig()->rcmap, (sizeof(((rxConfig_t *)0)->rcmap) / sizeof(((rxConfig_t *)0)->rcmap[0])), channel);


        uint16_t sample = (*rxRuntimeConfig.rcReadRawFn)(&rxRuntimeConfig, rawChannel);


        if (channel < 4 && sample != 0) {
            sample = scaleRange(sample, rxChannelRangeConfigs(channel)->min, rxChannelRangeConfigs(channel)->max, 1000, 2000);
            sample = ( __extension__ ({ __typeof__(( __extension__ ({ __typeof__(750) _left0 = (750); __typeof__(sample) _right1 = (sample); _left0 > _right1 ? _left0 : _right1; }))) _left2 = (( __extension__ ({ __typeof__(750) _left0 = (750); __typeof__(sample) _right1 = (sample); _left0 > _right1 ? _left0 : _right1; }))); __typeof__(2250) _right3 = (2250); _left2 < _right3 ? _left2 : _right3; }));
        }
        ...
        if (!isPulseValid(sample)) {
            ...
            if ((currentTimeMs > rcInvalidPulsPeriod[channel]) && (channel < 4)) {
                rxFlightChannelsValid = 
# 516 "./src/main/rx/rx.c" 3 4
                                       0
# 516 "./src/main/rx/rx.c"
                                            ;
            }
        } else {
    ...
    rcSampleIndex++;
    return 
# 548 "./src/main/rx/rx.c" 3 4
          1
# 548 "./src/main/rx/rx.c"
              ;
}
...
# 562 "./src/main/rx/rx.c" 3 4
                                                     _Bool 
# 562 "./src/main/rx/rx.c"
                                                          filtered)
{
    ...
    int rssiMin = rxConfig()->rssiMin * (1023/(float)100);
    int rssiMax = rxConfig()->rssiMax * (1023/(float)100);
    if (rssiMin > rssiMax) {
        int tmp = rssiMax;
        rssiMax = rssiMin;
        rssiMin = tmp;
    ...
    rssi = constrain(scaleRange(rssi, rssiMin, rssiMax, 0, 1023), 0, 1023);
}
...
static void updateRSSIPWM(void)
{
    ...
    if (rxConfig()->rssi_channel > 0) {
        ...
        uint16_t rawRSSI = (uint16_t)((constrain(pwmRssi - 1000, 0, 1000) / 1000.0f) * (1023 * 1.0f));
        setRSSI(rawRSSI, RSSI_SOURCE_RX_CHANNEL, 
# 629 "./src/main/rx/rx.c" 3 4
                                                0
# 629 "./src/main/rx/rx.c"
                                                     );
    }
}
...
static void updateRSSIADC(void)
{
    ...
    uint16_t rawRSSI = adcGetChannel(ADC_RSSI) / 4;
    setRSSI(rawRSSI, RSSI_SOURCE_ADC, 
# 637 "./src/main/rx/rx.c" 3 4
                                     0
# 637 "./src/main/rx/rx.c"
                                          );
...
void updateRSSI(timeUs_t currentTimeUs)
{
    ...
    case RSSI_SOURCE_MSP:
        if (cmpTimeUs(currentTimeUs, lastMspRssiUpdateUs) > 1500000) {
            rssi = 0;
        }
        break;
    default:
