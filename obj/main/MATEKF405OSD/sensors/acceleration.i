...
__attribute__ ((section(".fastram_bss"), aligned(4))) acc_t acc;

static uint16_t calibratingA = 0;

static __attribute__ ((section(".fastram_bss"), aligned(4))) int32_t accADC[3];

static __attribute__ ((section(".fastram_bss"), aligned(4))) biquadFilter_t accFilter[3];

static __attribute__ ((section(".fastram_bss"), aligned(4))) pt1Filter_t accVibeFloorFilter[3];
static __attribute__ ((section(".fastram_bss"), aligned(4))) pt1Filter_t accVibeFilter[3];


static __attribute__ ((section(".fastram_bss"), aligned(4))) filterApplyFnPtr accNotchFilterApplyFn;
static __attribute__ ((section(".fastram_bss"), aligned(4))) void *accNotchFilter[3];


extern void pgResetFn_accelerometerConfig(accelerometerConfig_t *); accelerometerConfig_t accelerometerConfig_System; accelerometerConfig_t accelerometerConfig_Copy; extern const pgRegistry_t accelerometerConfig_Registry; const pgRegistry_t accelerometerConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 35 | (2 << 12), .size = sizeof(accelerometerConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&accelerometerConfig_System, .copy = (uint8_t*)&accelerometerConfig_Copy, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_accelerometerConfig }, };

void pgResetFn_accelerometerConfig(accelerometerConfig_t *instance)
{
    *(instance) = (accelerometerConfig_t) { .acc_align = ALIGN_DEFAULT, .acc_hardware = ACC_AUTODETECT, .acc_lpf_hz = 15, .acc_notch_hz = 0, .acc_notch_cutoff = 1 };
     ...
     ;
    *(&instance->accZero) = (flightDynamicsTrims_t) { .raw[X] = 0, .raw[Y] = 0, .raw[Z] = 0 };



     ;
    *(&instance->accGain) = (flightDynamicsTrims_t) { .raw[X] = 4096, .raw[Y] = 4096, .raw[Z] = 4096 };
...
static 
# 109 "./src/main/sensors/acceleration.c" 3 4
      _Bool 
# 109 "./src/main/sensors/acceleration.c"
...
# 206 "./src/main/sensors/acceleration.c"
    case ACC_MPU6000:
...
# 282 "./src/main/sensors/acceleration.c"
    ...
    if (accHardware == ACC_NONE) {
        return 
# 291 "./src/main/sensors/acceleration.c" 3 4
              0
# 291 "./src/main/sensors/acceleration.c"
                   ;
    }
    ...
    return 
# 296 "./src/main/sensors/acceleration.c" 3 4
          1
# 296 "./src/main/sensors/acceleration.c"
              ;
}


# 299 "./src/main/sensors/acceleration.c" 3 4
_Bool 
# 299 "./src/main/sensors/acceleration.c"
...
# 331 "./src/main/sensors/acceleration.c" 3 4
_Bool 
# 331 "./src/main/sensors/acceleration.c"
    accIsCalibrationComplete(void)
{
...
static 
# 336 "./src/main/sensors/acceleration.c" 3 4
      _Bool 
# 336 "./src/main/sensors/acceleration.c"
...
static 
# 341 "./src/main/sensors/acceleration.c" 3 4
      _Bool 
# 341 "./src/main/sensors/acceleration.c"
...
static 
# 346 "./src/main/sensors/acceleration.c" 3 4
      _Bool 
# 346 "./src/main/sensors/acceleration.c"
           calibratedPosition[6];
static int32_t accSamples[6][3];
static int calibratedAxisCount = 0;

uint8_t accGetCalibrationAxisFlags(void)
{
    if (accIsCalibrationComplete() && (stateFlags & (ACCELEROMETER_CALIBRATED))) {
        return 0x3F;
    }

    static const uint8_t bitMap[6] = { 0, 1, 3, 5, 2, 4 };
    uint8_t flags = 0;
    for (int i = 0; i < 6; i++) {
...
static int getPrimaryAxisIndex(int32_t accADCData[3])
{
    ...
    if ((( __extension__ ({ __typeof__(sample[Z]) _abs0 = (sample[Z]); _abs0 < 0 ? -_abs0 : _abs0; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[X]) _abs1 = (sample[X]); _abs1 < 0 ? -_abs1 : _abs1; })) && (( __extension__ ({ __typeof__(sample[Z]) _abs2 = (sample[Z]); _abs2 < 0 ? -_abs2 : _abs2; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[Y]) _abs3 = (sample[Y]); _abs3 < 0 ? -_abs3 : _abs3; }))) {

        return (sample[Z] > 0) ? 0 : 1;
    }
    else if ((( __extension__ ({ __typeof__(sample[X]) _abs4 = (sample[X]); _abs4 < 0 ? -_abs4 : _abs4; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[Y]) _abs5 = (sample[Y]); _abs5 < 0 ? -_abs5 : _abs5; })) && (( __extension__ ({ __typeof__(sample[X]) _abs6 = (sample[X]); _abs6 < 0 ? -_abs6 : _abs6; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[Z]) _abs7 = (sample[Z]); _abs7 < 0 ? -_abs7 : _abs7; }))) {

        return (sample[X] > 0) ? 2 : 3;
    }
    else if ((( __extension__ ({ __typeof__(sample[Y]) _abs8 = (sample[Y]); _abs8 < 0 ? -_abs8 : _abs8; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[X]) _abs9 = (sample[X]); _abs9 < 0 ? -_abs9 : _abs9; })) && (( __extension__ ({ __typeof__(sample[Y]) _abs10 = (sample[Y]); _abs10 < 0 ? -_abs10 : _abs10; })) / 1.5f) > ( __extension__ ({ __typeof__(sample[Z]) _abs11 = (sample[Z]); _abs11 < 0 ? -_abs11 : _abs11; }))) {
...
static void performAcclerationCalibration(void)
{
    ...
    if (positionIndex == 0 && isOnFirstAccelerationCalibrationCycle()) {
        for (int axis = 0; axis < 6; axis++) {
            calibratedPosition[axis] = 
# 403 "./src/main/sensors/acceleration.c" 3 4
                                      0
# 403 "./src/main/sensors/acceleration.c"
                                           ;
            accSamples[axis][X] = 0;
            accSamples[axis][Y] = 0;
            accSamples[axis][Z] = 0;
        ...
        (stateFlags &= ~(ACCELEROMETER_CALIBRATED));
    }
    ...
    if (!calibratedPosition[positionIndex]) {
        ...
        if (isOnFinalAccelerationCalibrationCycle()) {
            calibratedPosition[positionIndex] = 
# 419 "./src/main/sensors/acceleration.c" 3 4
                                               1
# 419 "./src/main/sensors/acceleration.c"
                                                   ;

            accSamples[positionIndex][X] = accSamples[positionIndex][X] / 400;
            accSamples[positionIndex][Y] = accSamples[positionIndex][Y] / 400;
            accSamples[positionIndex][Z] = accSamples[positionIndex][Z] / 400;
...
static __attribute__ ((section(".fastram_bss"), aligned(4))) float accumulatedMeasurements[3];
static __attribute__ ((section(".fastram_bss"), aligned(4))) int accumulatedMeasurementCount;
...
void accGetMeasuredAcceleration(fpVector3_t *measuredAcc)
{
    ...
    if (accumulatedMeasurementCount) {
        for (int axis = 0; axis < 3; axis++) {
            measuredAcc->v[axis] = accumulatedMeasurements[axis] * 980.665f / accumulatedMeasurementCount;
        ...
        for (int axis = 0; axis < 3; axis++) {
            measuredAcc->v[axis] = acc.accADCf[axis] * 980.665f;
        }
    }
...
void accUpdate(void)
{
    ...
    if (( __extension__ ({ __typeof__(acc.accADCf[X]) _abs12 = (acc.accADCf[X]); _abs12 < 0 ? -_abs12 : _abs12; })) > 7.9f || ( __extension__ ({ __typeof__(acc.accADCf[Y]) _abs13 = (acc.accADCf[Y]); _abs13 < 0 ? -_abs13 : _abs13; })) > 7.9f || ( __extension__ ({ __typeof__(acc.accADCf[Z]) _abs14 = (acc.accADCf[Z]); _abs14 < 0 ? -_abs14 : _abs14; })) > 7.9f) {
        acc.accClipCount++;
    }
...
void accInitFilters(void)
{
    ...
    for (int axis = 0; axis < 3; axis++) {
        pt1FilterInit(&accVibeFloorFilter[axis], 5.0f, accDt);
        pt1FilterInit(&accVibeFilter[axis], 2.0f, accDt);
    }


    static __attribute__ ((section(".fastram_bss"), aligned(4))) biquadFilter_t accFilterNotch[3];
...
# 635 "./src/main/sensors/acceleration.c" 3 4
_Bool 
# 635 "./src/main/sensors/acceleration.c"
    accIsHealthy(void)
{
    return 
# 637 "./src/main/sensors/acceleration.c" 3 4
          1
# 637 "./src/main/sensors/acceleration.c"
              ;
}
