...
# 27 "./src/main/config/config_eeprom.h" 3 4
_Bool 
# 27 "./src/main/config/config_eeprom.h"
    loadEEPROM(void);
...
# 30 "./src/main/drivers/rx_spi.h" 3 4
_Bool 
# 30 "./src/main/drivers/rx_spi.h"
       rxSpiCheckIrq(void);
...
# 21 "./src/main/drivers/pwm_output.h" 2
...
# 39 "./src/main/drivers/pwm_output.h"
typedef enum {
    PWM_TYPE_STANDARD = 0,
    PWM_TYPE_ONESHOT125,
    PWM_TYPE_ONESHOT42,
...
# 55 "./src/main/drivers/pwm_output.h" 3 4
_Bool 
# 55 "./src/main/drivers/pwm_output.h"
    pwmMotorConfig(const struct timerHardware_s *timerHardware, uint8_t motorIndex, uint16_t motorPwmRate, uint16_t idlePulse, motorPwmProtocolTypes_e proto, 
# 55 "./src/main/drivers/pwm_output.h" 3 4
                                                                                                                                                              _Bool 
# 55 "./src/main/drivers/pwm_output.h"
                                                                                                                                                                   enableOutput);

# 56 "./src/main/drivers/pwm_output.h" 3 4
_Bool 
# 56 "./src/main/drivers/pwm_output.h"
    pwmServoConfig(const struct timerHardware_s *timerHardware, uint8_t servoIndex, uint16_t servoPwmRate, uint16_t servoCenterPulse, 
# 56 "./src/main/drivers/pwm_output.h" 3 4
                                                                                                                                      _Bool 
# 56 "./src/main/drivers/pwm_output.h"
                                                                                                                                           enableOutput);
void pwmWriteBeeper(
# 57 "./src/main/drivers/pwm_output.h" 3 4
                   _Bool 
# 57 "./src/main/drivers/pwm_output.h"
                        onoffBeep);
...
# 22 "./src/main/sensors/battery.h" 2
...
const featureConfig_t pgResetTemplate_featureConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .enabledFeatures = (FEATURE_OSD | FEATURE_CURRENT_METER | FEATURE_VBAT | FEATURE_TELEMETRY ) | (FEATURE_TX_PROF_SEL) }
...
const systemConfig_t pgResetTemplate_systemConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .current_profile_index = 0, .current_battery_profile_index = 0, .debug_mode = DEBUG_NONE, .i2c_speed = I2C_SPEED_400KHZ, .cpuUnderclock = 0, .accTaskFrequency = 500, .attitudeTaskFrequency = 250, .asyncMode = ASYNC_MODE_NONE, .throttle_tilt_compensation_strength = 0, .pwmRxInputFilteringMode = INPUT_FILTERING_DISABLED, .name = { 0 } }
# 123 "./src/main/fc/config.c"
 ;

beeperConfig_t beeperConfig_System; beeperConfig_t beeperConfig_Copy; extern const pgRegistry_t beeperConfig_Registry; const pgRegistry_t beeperConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 1005 | (0 << 12), .size = sizeof(beeperConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&beeperConfig_System, .copy = (uint8_t*)&beeperConfig_Copy, .ptr = 0, .reset = {.ptr = 0}, };
...
const adcChannelConfig_t pgResetTemplate_adcChannelConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .adcFunctionChannel = { [ADC_BATTERY] = ADC_CHN_1, [ADC_RSSI] = ADC_CHN_3, [ADC_CURRENT] = ADC_CHN_2, [ADC_AIRSPEED] = ADC_CHN_NONE, } }
...
void validateNavConfig(void)
{
    ...
    navConfigMutable()->general.land_slowdown_minalt = ( __extension__ ({ __typeof__(navConfig()->general.land_slowdown_minalt) _left0 = (navConfig()->general.land_slowdown_minalt); __typeof__(navConfig()->general.land_slowdown_maxalt - 100) _right1 = (navConfig()->general.land_slowdown_maxalt - 100); _left0 < _right1 ? _left0 : _right1; }));
}
...
# 242 "./src/main/fc/config.c"
    ...
    if (featureConfigured(FEATURE_SOFTSERIAL) && featureConfigured(FEATURE_LED_STRIP)) {
        const timerHardware_t *ledTimerHardware = timerGetByTag(((((0) + 1) << 4) | (15)), TIM_USE_ANY);
        if (ledTimerHardware != 
# 264 "./src/main/fc/config.c" 3 4
                               ((void *)0)
# 264 "./src/main/fc/config.c"
                                   ) {
            
# 265 "./src/main/fc/config.c" 3 4
           _Bool 
# 265 "./src/main/fc/config.c"
                sameTimerUsed = 
# 265 "./src/main/fc/config.c" 3 4
                                0
# 265 "./src/main/fc/config.c"
            ...
            const timerHardware_t *ss1TimerHardware = timerGetByTag(((((0) + 1) << 4) | (1)), TIM_USE_ANY);
            if (ss1TimerHardware != 
# 269 "./src/main/fc/config.c" 3 4
                                   ((void *)0) 
# 269 "./src/main/fc/config.c"
                                        && ss1TimerHardware->tim == ledTimerHardware->tim) {
                sameTimerUsed = 
# 270 "./src/main/fc/config.c" 3 4
                               1
# 270 "./src/main/fc/config.c"
                                   ;
            }


            const timerHardware_t *ss2TimerHardware = timerGetByTag(((((0) + 1) << 4) | (2)), TIM_USE_ANY);
            if (ss2TimerHardware != 
# 275 "./src/main/fc/config.c" 3 4
                                   ((void *)0) 
# 275 "./src/main/fc/config.c"
                                        && ss2TimerHardware->tim == ledTimerHardware->tim) {
                sameTimerUsed = 
# 276 "./src/main/fc/config.c" 3 4
                               1
# 276 "./src/main/fc/config.c"
                                   ;
            }
    ...
    if (!isSerialConfigValid(serialConfigMutable())) {
        pgResetCopy(serialConfigMutable(), 13);
    }
    ...
    case PWM_TYPE_STANDARD:
        motorConfigMutable()->motorPwmRate = ( __extension__ ({ __typeof__(motorConfig()->motorPwmRate) _left2 = (motorConfig()->motorPwmRate); __typeof__(490) _right3 = (490); _left2 < _right3 ? _left2 : _right3; }));
        break;
    case PWM_TYPE_ONESHOT125:
        motorConfigMutable()->motorPwmRate = ( __extension__ ({ __typeof__(motorConfig()->motorPwmRate) _left4 = (motorConfig()->motorPwmRate); __typeof__(3900) _right5 = (3900); _left4 < _right5 ? _left4 : _right5; }));
        break;
    ...
    if (settingsValidate(
# 328 "./src/main/fc/config.c" 3 4
                        ((void *)0)
# 328 "./src/main/fc/config.c"
                            )) {
        (armingFlags &= ~(ARMING_DISABLED_INVALID_SETTING));
    } else {
        (armingFlags |= (ARMING_DISABLED_INVALID_SETTING));
    }
}
...
# 451 "./src/main/fc/config.c" 3 4
_Bool 
# 451 "./src/main/fc/config.c"
    setConfigProfile(uint8_t profileIndex)
{
    
# 453 "./src/main/fc/config.c" 3 4
   _Bool 
# 453 "./src/main/fc/config.c"
        ret = 
# 453 "./src/main/fc/config.c" 3 4
              1
# 453 "./src/main/fc/config.c"
    ...
    if (systemConfig()->current_profile_index == profileIndex) {
        ret = 
# 455 "./src/main/fc/config.c" 3 4
              0
# 455 "./src/main/fc/config.c"
                   ;
    }
    if (profileIndex >= 3) {
        profileIndex = 0;
...
# 482 "./src/main/fc/config.c" 3 4
_Bool 
# 482 "./src/main/fc/config.c"
    setConfigBatteryProfile(uint8_t profileIndex)
{
    
# 484 "./src/main/fc/config.c" 3 4
   _Bool 
# 484 "./src/main/fc/config.c"
        ret = 
# 484 "./src/main/fc/config.c" 3 4
              1
# 484 "./src/main/fc/config.c"
    ...
    if (systemConfig()->current_battery_profile_index == profileIndex) {
        ret = 
# 486 "./src/main/fc/config.c" 3 4
              0
# 486 "./src/main/fc/config.c"
                   ;
    }
    if (profileIndex >= 3) {
        profileIndex = 0;
