...
static __attribute__ ((section(".fastram_bss"), aligned(4))) filterApplyFnPtr notchFilterApplyFn;


extern float dT;

__attribute__ ((section(".fastram_bss"), aligned(4))) float headingHoldCosZLimit;
__attribute__ ((section(".fastram_bss"), aligned(4))) int16_t headingHoldTarget;
static __attribute__ ((section(".fastram_bss"), aligned(4))) pt1Filter_t headingHoldRateFilter;
static __attribute__ ((section(".fastram_bss"), aligned(4))) pt1Filter_t fixedWingTpaFilter;


static __attribute__ ((section(".fastram_bss"), aligned(4))) 
# 101 "./src/main/flight/pid.c" 3 4
              _Bool 
# 101 "./src/main/flight/pid.c"
                   pidGainsUpdateRequired;
__attribute__ ((section(".fastram_bss"), aligned(4))) int16_t axisPID[3];


int32_t axisPID_P[3], axisPID_I[3], axisPID_D[3], axisPID_Setpoint[3];


static __attribute__ ((section(".fastram_bss"), aligned(4))) pidState_t pidState[3];

extern const pidProfile_t pgResetTemplate_pidProfile; static pidProfile_t pidProfile_Storage[3]; static pidProfile_t pidProfile_CopyStorage[3]; pidProfile_t *pidProfile_ProfileCurrent; extern const pgRegistry_t pidProfile_Registry; const pgRegistry_t pidProfile_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 14 | (4 << 12), .size = sizeof(pidProfile_t) | PGR_SIZE_PROFILE_FLAG, .address = (uint8_t*)&pidProfile_Storage, .copy = (uint8_t*)&pidProfile_CopyStorage, .ptr = (uint8_t **)&pidProfile_ProfileCurrent, .reset = {.ptr = (void*)&pgResetTemplate_pidProfile}, };

const pidProfile_t pgResetTemplate_pidProfile __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .bank_mc = { .pid = { [PID_ROLL] = { 40, 30, 23 }, [PID_PITCH] = { 40, 30, 23 }, [PID_YAW] = { 85, 45, 0 }, [PID_LEVEL] = { .P = 20, .I = 15, .D = 75, }, [PID_HEADING] = { 60, 0, 0 }, [PID_POS_XY] = { .P = 65, .I = 120, .D = 10, }, [PID_VEL_XY] = { .P = 40, .I = 15, .D = 100, }, [PID_POS_Z] = { .P = 50, .I = 0, .D = 0, }, [PID_VEL_Z] = { .P = 100, .I = 50, .D = 10, } } }, .bank_fw = { .pid = { [PID_ROLL] = { 5, 7, 50 }, [PID_PITCH] = { 5, 7, 50 }, [PID_YAW] = { 6, 10, 60 }, [PID_LEVEL] = { .P = 20, .I = 5, .D = 75, }, [PID_HEADING] = { 60, 0, 0 }, [PID_POS_Z] = { .P = 40, .I = 5, .D = 10, }, [PID_POS_XY] = { .P = 75, .I = 5, .D = 8, } } }, .acc_soft_lpf_hz = 15, .dterm_soft_notch_hz = 0, .dterm_soft_notch_cutoff = 1, .dterm_lpf_hz = 40, .yaw_lpf_hz = 30, .dterm_setpoint_weight = 1.0f, .itermWindupPointPercent = 50, .axisAccelerationLimitYaw = 10000, .axisAccelerationLimitRollPitch = 0, .yaw_p_limit = 300, .heading_hold_rate_limit = 90, .max_angle_inclination[FD_ROLL] = 300, .max_angle_inclination[FD_PITCH] = 300, .pidSumLimit = 500, .fixedWingItermThrowLimit = 165, .fixedWingReferenceAirspeed = 1000, .fixedWingCoordinatedYawGain = 1.0f, }
# 194 "./src/main/flight/pid.c"
 ;
...
void pidInit(void)
{
    ...
    static const float dtermCoeffs[5] = {5.0f/8, 2.0f/8, -8.0f/8, -2.0f/8, 3.0f/8};
    for (int axis = 0; axis < 3; ++ axis) {
        firFilterInit(&pidState[axis].gyroRateFilter, pidState[axis].gyroRateBuf, 5, dtermCoeffs);
    }

    pidResetTPAFilter();


    headingHoldCosZLimit = cos_approx((((pidProfile()->max_angle_inclination[FD_ROLL]) / 10.0f) * (3.14159265358979323846f / 180.0f))) *
                           cos_approx((((pidProfile()->max_angle_inclination[FD_PITCH]) / 10.0f) * (3.14159265358979323846f / 180.0f)));

    pidGainsUpdateRequired = 
# 212 "./src/main/flight/pid.c" 3 4
                            0
# 212 "./src/main/flight/pid.c"
                                 ;
}


# 215 "./src/main/flight/pid.c" 3 4
_Bool 
# 215 "./src/main/flight/pid.c"
    pidInitFilters(void)
{
    ...
    if (refreshRate == 0) {
        return 
# 221 "./src/main/flight/pid.c" 3 4
              0
# 221 "./src/main/flight/pid.c"
                   ;
    }
    ...
    return 
# 238 "./src/main/flight/pid.c" 3 4
          1
# 238 "./src/main/flight/pid.c"
              ;
}

void pidResetTPAFilter(void)
{
    if ((stateFlags & (FIXED_WING)) && currentControlRateProfile->throttle.fixedWingTauMs > 0) {
...
{
    pidGainsUpdateRequired = 
# 333 "./src/main/flight/pid.c" 3 4
                            1
# 333 "./src/main/flight/pid.c"
                                ;
}

void updatePIDCoefficients(void)
{
    static __attribute__ ((section(".fastram_bss"), aligned(4))) uint16_t prevThrottle = 0;
    ...
    if ((stateFlags & (FIXED_WING)) && (currentControlRateProfile->throttle.fixedWingTauMs > 0)) {
        ...
        if (filteredThrottle != prevThrottle) {
            ...
            pidGainsUpdateRequired = 
# 345 "./src/main/flight/pid.c" 3 4
                                    1
# 345 "./src/main/flight/pid.c"
                                        ;
        }
    }
    else {
        if (rcCommand[THROTTLE] != prevThrottle) {
            ...
            pidGainsUpdateRequired = 
# 351 "./src/main/flight/pid.c" 3 4
                                    1
# 351 "./src/main/flight/pid.c"
                                        ;
        }
    }
    ...
    const float tpaFactor = (stateFlags & (FIXED_WING)) ? calculateFixedWingTPAFactor(prevThrottle) : calculateMultirotorTPAFactor();
    ...
    for (int axis = 0; axis < 3; axis++) {
        if ((stateFlags & (FIXED_WING))) {
            ...
            pidState[axis].kP = pidBank()->pid[axis].P / 31.0f * tpaFactor;
            pidState[axis].kI = pidBank()->pid[axis].I / 4.0f * tpaFactor;
            pidState[axis].kD = 0.0f;
            pidState[axis].kFF = pidBank()->pid[axis].D / 31.0f * tpaFactor;
            pidState[axis].kT = 0.0f;
        }
        else {
            const float axisTPA = (axis == FD_YAW) ? 1.0f : tpaFactor;
            pidState[axis].kP = pidBank()->pid[axis].P / 31.0f * axisTPA;
            pidState[axis].kI = pidBank()->pid[axis].I / 4.0f;
            pidState[axis].kD = pidBank()->pid[axis].D / 1905.0f * axisTPA;
    ...
    pidGainsUpdateRequired = 
# 389 "./src/main/flight/pid.c" 3 4
                            0
# 389 "./src/main/flight/pid.c"
                                 ;
}
...
static float calcHorizonRateMagnitude(void)
{
    ...
    const int32_t stickPosAil = ( __extension__ ({ __typeof__(getRcStickDeflection(FD_ROLL)) _abs0 = (getRcStickDeflection(FD_ROLL)); _abs0 < 0 ? -_abs0 : _abs0; }));
    const int32_t stickPosEle = ( __extension__ ({ __typeof__(getRcStickDeflection(FD_PITCH)) _abs1 = (getRcStickDeflection(FD_PITCH)); _abs1 < 0 ? -_abs1 : _abs1; }));
    const float mostDeflectedStickPos = constrain(( __extension__ ({ __typeof__(stickPosAil) _left2 = (stickPosAil); __typeof__(stickPosEle) _right3 = (stickPosEle); _left2 > _right3 ? _left2 : _right3; })), 0, 500) / 500.0f;
...
static void pidLevel(pidState_t *pidState, flight_dynamics_index_t axis, float horizonRateMagnitude)
{
    ...
    if ((axis == FD_PITCH) && (stateFlags & (FIXED_WING)) && (flightModeFlags & (ANGLE_MODE)) && !navigationIsControllingThrottle())
        angleTarget += scaleRange(( __extension__ ({ __typeof__(0) _left4 = (0); __typeof__(navConfig()->fw.cruise_throttle - rcCommand[THROTTLE]) _right5 = (navConfig()->fw.cruise_throttle - rcCommand[THROTTLE]); _left4 > _right5 ? _left4 : _right5; })), 0, navConfig()->fw.cruise_throttle - 1000, 0, mixerConfig()->fwMinThrottleDownPitchAngle);

    const float angleErrorDeg = ((angleTarget - attitude.raw[axis]) / 10);

    float angleRateTarget = constrainf(angleErrorDeg * (pidBank()->pid[PID_LEVEL].P / 6.56f), -currentControlRateProfile->stabilized.rates[axis] * 10.0f, currentControlRateProfile->stabilized.rates[axis] * 10.0f);
# 437 "./src/main/flight/pid.c"
...
static void pidApplyFixedWingRateController(pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    if ((stateFlags & (ANTI_WINDUP))) {
        pidState->errorGyroIf = constrainf(pidState->errorGyroIf, -pidState->errorGyroIfLimit, pidState->errorGyroIfLimit);
    } else {
        pidState->errorGyroIfLimit = ( __extension__ ({ __typeof__(pidState->errorGyroIf) _abs6 = (pidState->errorGyroIf); _abs6 < 0 ? -_abs6 : _abs6; }));
    }
    ...
    if ((flightModeFlags & (AUTO_TUNE)) && !(flightModeFlags & (MANUAL_MODE))) {
...
static void pidApplyMulticopterRateController(pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    if ((stateFlags & (ANTI_WINDUP)) || mixerIsOutputSaturated()) {
        pidState->errorGyroIf = constrainf(pidState->errorGyroIf, -pidState->errorGyroIfLimit, pidState->errorGyroIfLimit);
    } else {
        pidState->errorGyroIfLimit = ( __extension__ ({ __typeof__(pidState->errorGyroIf) _abs7 = (pidState->errorGyroIf); _abs7 < 0 ? -_abs7 : _abs7; }));
    }
...
static uint8_t getHeadingHoldState(void)
{
    ...
    if (( __extension__ ({ __typeof__(rcCommand[YAW]) _abs8 = (rcCommand[YAW]); _abs8 < 0 ? -_abs8 : _abs8; })) == 0 && (flightModeFlags & (HEADING_MODE))) {
        return HEADING_HOLD_ENABLED;
    } else {
...
float pidHeadingHold(void)
{
    ...
    int16_t error = ((attitude.values.yaw) / 10) - headingHoldTarget;
...
# 665 "./src/main/flight/pid.c"
    ...
    headingHoldRate = pt1FilterApply4(&headingHoldRateFilter, headingHoldRate, 2, dT);
...
# 693 "./src/main/flight/pid.c"
            ...
            float bankAngle = (((attitude.values.roll) / 10.0f) * (3.14159265358979323846f / 180.0f));
            float coordinatedTurnRateEarthFrame = 980.665f * (sin_approx(-bankAngle) / cos_approx(-bankAngle)) / airspeedForCoordinatedTurn;

            targetRates.z = ((coordinatedTurnRateEarthFrame) / (3.14159265358979323846f / 180.0f));
        }
        else {
...
void pidController(void)
{
    ...
    if (headingHoldState == HEADING_HOLD_UPDATE_HEADING) {
        updateHeadingHoldTarget(((attitude.values.yaw) / 10));
    }
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        pidState[axis].rateTarget = constrainf(rateTarget, -1800, +1800);
    }
    ...
    if ((flightModeFlags & (TURN_ASSISTANT)) || navigationRequiresTurnAssistance()) {
