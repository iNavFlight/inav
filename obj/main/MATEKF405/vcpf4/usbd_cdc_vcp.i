# 1 "./src/main/vcpf4/usbd_cdc_vcp.c"
# 1 "/home/osboxes/inav//"
# 1 "<built-in>"
#define __STDC__ 1
...
# 1 "./src/main/vcpf4/usbd_cdc_vcp.c"
# 27 "./src/main/vcpf4/usbd_cdc_vcp.c"
...
# 28 "./src/main/vcpf4/usbd_cdc_vcp.c" 2
...
# 30 "./src/main/vcpf4/usbd_cdc_vcp.c" 2
...
# 31 "./src/main/vcpf4/usbd_cdc_vcp.c" 2
...
static uint8_t APP_Tx_Buffer[2048];
...
# 76 "./src/main/vcpf4/usbd_cdc_vcp.c"
static uint16_t VCP_Init(void)
{
...
# 112 "./src/main/vcpf4/usbd_cdc_vcp.c"
static uint16_t VCP_Ctrl(uint32_t Cmd, uint8_t* Buf, uint32_t Len)
{
...
# 177 "./src/main/vcpf4/usbd_cdc_vcp.c"
uint32_t CDC_Send_DATA(const uint8_t *ptrBuffer, uint32_t sendLength)
{
...
uint32_t CDC_Send_FreeBytes(void)
{
    ...
    return ((APP_Rx_ptr_out - APP_Rx_ptr_in) + (-((int)(APP_Rx_ptr_out <= APP_Rx_ptr_in)) & 2048)) - 1;
}
# 202 "./src/main/vcpf4/usbd_cdc_vcp.c"
static uint16_t VCP_DataTx(const uint8_t* Buf, uint32_t Len)
{
    ...
    for (uint32_t i = 0; i < Len; i++) {
        ...
        APP_Rx_ptr_in = (APP_Rx_ptr_in + 1) % 2048;
...
# 230 "./src/main/vcpf4/usbd_cdc_vcp.c"
uint32_t CDC_Receive_DATA(uint8_t* recvBuf, uint32_t len)
{
    uint32_t count = 0;
    ...
    while (APP_Tx_ptr_out != APP_Tx_ptr_in && count < len) {
        ...
        APP_Tx_ptr_out = (APP_Tx_ptr_out + 1) % 2048;
        count++;
    }
    return count;
}
...
uint32_t CDC_Receive_BytesAvailable(void)
{
    ...
    return APP_Tx_ptr_out > APP_Tx_ptr_in ? 2048 - APP_Tx_ptr_out + APP_Tx_ptr_in : APP_Tx_ptr_in - APP_Tx_ptr_out;
}
# 263 "./src/main/vcpf4/usbd_cdc_vcp.c"
static uint16_t VCP_DataRx(uint8_t* Buf, uint32_t Len)
{
    if (CDC_Receive_BytesAvailable() + Len > 2048) {
        return USBD_FAIL;
    }

    for (uint32_t i = 0; i < Len; i++) {
        APP_Tx_Buffer[APP_Tx_ptr_in] = Buf[i];
        APP_Tx_ptr_in = (APP_Tx_ptr_in + 1) % 2048;
    }

    return USBD_OK;
}
# 284 "./src/main/vcpf4/usbd_cdc_vcp.c"
uint8_t usbIsConfigured(void)
{
    return (bDeviceState == CONFIGURED);
}
# 296 "./src/main/vcpf4/usbd_cdc_vcp.c"
uint8_t usbIsConnected(void)
{
    return (bDeviceState != UNCONNECTED);
}
# 308 "./src/main/vcpf4/usbd_cdc_vcp.c"
uint32_t CDC_BaudRate(void)
{
    return g_lc.bitrate;
}
# 320 "./src/main/vcpf4/usbd_cdc_vcp.c"
void CDC_SetBaudRateCb(void (*cb)(void *context, uint32_t baud), void *context)
{
    baudRateCbContext = context;
    baudRateCb = cb;
}
# 333 "./src/main/vcpf4/usbd_cdc_vcp.c"
void CDC_SetCtrlLineStateCb(void (*cb)(void *context, uint16_t ctrlLineState), void *context)
{
