...
static 
# 85 "./src/main/telemetry/ltm.c" 3 4
      _Bool 
# 85 "./src/main/telemetry/ltm.c"
           ltmEnabled;
static portSharing_e ltmPortSharing;
static uint8_t ltmFrame[(14 +4)];
static uint8_t ltm_x_counter;

static void ltm_initialise_packet(sbuf_t *dst)
{
    dst->ptr = ltmFrame;
    dst->end = (&(ltmFrame)[(sizeof(ltmFrame) / sizeof((ltmFrame)[0]))]);
...
void ltm_gframe(sbuf_t *dst)
{
    ...
    ltm_alt = getEstimatedActualPosition(Z);
...
void ltm_sframe(sbuf_t *dst)
{
    ...
    if ((flightModeFlags & (MANUAL_MODE)))
        ...
        lt_flightmode = 18;
    else if ((flightModeFlags & (NAV_ALTHOLD_MODE)))
        lt_flightmode = 8;
    else if ((flightModeFlags & (HEADFREE_MODE)) || (flightModeFlags & (HEADING_MODE)))
        lt_flightmode = 11;
    ...
    uint8_t lt_statemode = ((armingFlags & (ARMED))) ? 1 : 0;
    if (failsafeIsActive())
        lt_statemode |= 2;
    sbufWriteU8(dst, 'S');
    sbufWriteU16(dst, getBatteryVoltage() * 10);
    sbufWriteU16(dst, (uint16_t)constrain(getMAhDrawn(), 0, 0xFFFF));
    sbufWriteU8(dst, (uint8_t)((getRSSI() * 254) / 1023));

    sbufWriteU8(dst, sensors(SENSOR_PITOT) ? pitot.airSpeed / 100.0f : 0);
...
void ltm_oframe(sbuf_t *dst)
{
    ...
    sbufWriteU8(dst, 1);
    sbufWriteU8(dst, (stateFlags & (GPS_FIX_HOME)) ? 1 : 0);
}
...
static uint8_t ltm_normal_schedule[(1000/100)] = {
    (1 << 0) | (1 << 1),
    (1 << 0) | (1 << 2) | (1 << 3),
    (1 << 0) | (1 << 1),
    (1 << 0) | (1 << 2) | (1 << 4),
...
static uint8_t ltm_medium_schedule[(1000/100)] = {
    (1 << 0),
    (1 << 1),
    (1 << 0) | (1 << 2),
    (1 << 3),
...
static uint8_t ltm_slow_schedule[(1000/100)] = {
    (1 << 1),
    (1 << 2),
    (1 << 0),
    0,
...
static void process_ltm(void)
{
    ...
    if (current_schedule & (1 << 3)) {
        ltm_initialise_packet(dst);
        ltm_oframe(dst);
        ltm_finalise(dst);
    }

    if (current_schedule & (1 << 5)) {
        ltm_initialise_packet(dst);
        ltm_xframe(dst);
        ltm_finalise(dst);
    }
...
void freeLtmTelemetryPort(void)
{
    ...
    ltmPort = 
# 394 "./src/main/telemetry/ltm.c" 3 4
             ((void *)0)
# 394 "./src/main/telemetry/ltm.c"
                 ;
    ltmEnabled = 
# 395 "./src/main/telemetry/ltm.c" 3 4
                0
# 395 "./src/main/telemetry/ltm.c"
                     ;
}
...
# 435 "./src/main/telemetry/ltm.c" 3 4
                                                                            ((void *)0)
# 435 "./src/main/telemetry/ltm.c"
                                                                                , 
# 435 "./src/main/telemetry/ltm.c" 3 4
                                                                                  ((void *)0)
# 435 "./src/main/telemetry/ltm.c"
                                                                                      , baudRates[baudRateIndex], MODE_TX, SERIAL_NOT_INVERTED);
    if (!ltmPort)
        return;
    ltm_x_counter = 0;
    ltmEnabled = 
# 439 "./src/main/telemetry/ltm.c" 3 4
                1
# 439 "./src/main/telemetry/ltm.c"
                    ;
}
...
# 445 "./src/main/telemetry/ltm.c" 3 4
                                                 ((void *)0)
# 445 "./src/main/telemetry/ltm.c"
                                                     ) {
            ltmPort = telemetrySharedPort;
            configureLtmScheduler();
            ltmEnabled = 
# 448 "./src/main/telemetry/ltm.c" 3 4
                        1
# 448 "./src/main/telemetry/ltm.c"
                            ;
        }
    } else {
        
# 451 "./src/main/telemetry/ltm.c" 3 4
       _Bool 
# 451 "./src/main/telemetry/ltm.c"
...
int getLtmFrame(uint8_t *frame, ltm_frame_e ltmFrameType)
{
    ...
    sbuf_t ltmFrameBuf = { .ptr = ltmFrame, .end =(&(ltmFrame)[(sizeof(ltmFrame) / sizeof((ltmFrame)[0]))]) };
